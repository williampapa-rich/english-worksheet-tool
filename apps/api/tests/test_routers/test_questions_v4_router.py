"""POST /questions/{question_id}/variants/grammar-inline 라우터 단위 테스트.

mock session + mock repository + mock LLM client 로 실제 DB / LLM 없이 실행한다.

커버 케이스:
  - 201 정상 — grammar_29 원본 → V4 변형 Question 반환.
  ...
  - 422 — V4 비적용 type (vocabulary_30 — V2 대상).
  - 422 — V4 비적용 type (gist_22 — V6 대상).
  - 422 — V4 비적용 type (blank_phrase_31 — V2/V5 대상).
...
_GRAMMAR_PASSAGE_TEXT = (
    "The number of students who choose online learning has grown significantly over the past "
    "decade. Institutions that fail to adapt their curricula risk losing relevance."
)

_MODIFIED_PASSAGE = (
    "The number of students who choose online learning (A) [has grown / have grown] "
    "significantly over the past decade. Institutions that fail "
    "(B) [to adapt / adapting] their curricula risk losing relevance."
)
...
def _make_passage(
    ...
    return Passage(
        ...
        tenant_id=tenant_id,
        workspace_id=workspace_id,
        body_text=_GRAMMAR_PASSAGE_TEXT,
        word_count=25,
...
def _make_saved_variant() -> Question:
    return Question(
        ...
        type=QuestionType.GRAMMAR_29,
        variant_kind=VariantKind.GRAMMAR_INLINE,
        question_text="밑줄 친 (A), (B)에 들어갈 말로 어법에 맞는 것은?",
        choices=_FIVE_CHOICES,
        answer=1,
        explanation=(
            "(A)의 주어는 'The number'(단수)이므로 has grown이 옳다. "
            "(B)의 fail은 to부정사를 목적어로 취하므로 to adapt가 정확하다."
        ),
        variant_metadata={
...
def prompts_dir(tmp_path: Path) -> Path:
    """임시 프롬프트 디렉토리 — V4 + 기존 라우트 프롬프트 포함."""
    d = tmp_path / "prompts"
    d.mkdir()
    (d / "variant-grammar-inline-v0.md").write_text(
        "---\nversion: 0\n---\nPassage: {{passage_text}}\nBox count: {{box_count}}",
        encoding="utf-8",
    )
    # 다른 라우트 프롬프트도 포함 (import 에러 방지)
...
@pytest.mark.asyncio
async def test_create_v4_variant_grammar_29_ok(
    ...
    data = response.json()
    assert data["variant_kind"] == "grammar_inline"
    assert data["type"] == "grammar_29"
    assert len(data["choices"]) == 5


@pytest.mark.asyncio
async def test_create_v4_variant_qa_placeholder_created(
    async_client: AsyncClient,
    mock_session: AsyncMock,
    mock_llm_client: AsyncMock,
) -> None:
...
@pytest.mark.asyncio
async def test_create_v4_variant_question_not_found_404(
    async_client: AsyncClient,
    mock_session: AsyncMock,
) -> None:
    """404 — 존재하지 않는 question_id."""
...
@pytest.mark.asyncio
async def test_create_v4_variant_cross_tenant_404(
    async_client: AsyncClient,
    mock_session: AsyncMock,
) -> None:
...
@pytest.mark.asyncio
async def test_create_v4_variant_passage_not_found_404(
    async_client: AsyncClient,
    mock_session: AsyncMock,
) -> None:
...
@pytest.mark.asyncio
async def test_create_v4_variant_vocabulary_30_non_applicable_422(
    async_client: AsyncClient,
    mock_session: AsyncMock,
) -> None:
    """422 — V4 비적용 type (vocabulary_30 — V2 대상)."""
    ...
    assert "V4 변형" in response.json()["detail"]


@pytest.mark.asyncio
async def test_create_v4_variant_gist_22_non_applicable_422(
    async_client: AsyncClient,
    mock_session: AsyncMock,
) -> None:
    """422 — V4 비적용 type (gist_22 — V6 대상)."""
...
@pytest.mark.asyncio
async def test_create_v4_variant_blank_phrase_31_non_applicable_422(
    async_client: AsyncClient,
    mock_session: AsyncMock,
) -> None:
    """422 — V4 비적용 type (blank_phrase_31 — V2/V5 대상)."""
...
@pytest.mark.asyncio
async def test_create_v4_variant_llm_schema_error_502(
    async_client: AsyncClient,
    mock_session: AsyncMock,
) -> None:
    """502 — LLMSchemaValidationError."""
...
@pytest.mark.asyncio
async def test_create_v4_variant_llm_timeout_504(
    async_client: AsyncClient,
    mock_session: AsyncMock,
) -> None:
    """504 — LLMTimeoutError."""
...
@pytest.mark.asyncio
async def test_create_v4_variant_permanent_llm_error_500(
    async_client: AsyncClient,
    mock_session: AsyncMock,
) -> None:
    """500 — PermanentLLMError."""
