...
카탈로그 v0.4 §V2 검증 기준:
  ...
  - choices 5개.
  - 오답 옵션이 dictionary 동의어 쌍 회피 (프롬프트 가이드 반영, Pydantic 에서
    구조만 검증 — 의미 적합성은 qa-validator 별도).
...
# ─── 프롬프트 템플릿 ID ─────────────────────────────────────────────────────
V2_PROMPT_ID = "variant-vocabulary-inline-v0"

# V2 적용 가능 QuestionType (카탈로그 v0.4 §V2 — 확실)
V2_APPLICABLE_TYPES: frozenset[QuestionType] = frozenset(
    {
...
# 인라인 박스 개수 범위 (카탈로그 v0.4 §V2 — 2~3개)
_MIN_BOX_COUNT = 2
_MAX_BOX_COUNT = 3
...
class V2InlineChoiceOutput(BaseModel):
    """V2 LLM structured output 안의 개별 inline_choice 항목.

    docs/prompts/variant-vocabulary-inline-v0.md 의 inline_choices 배열 원소.
    ...
    options: list[str] = Field(
        ...
        min_length=2,
        max_length=2,
        description="[correct_word, distractor_word] — options[0] 이 항상 정답.",
    )
    ...
    kind: str = Field(
        default="vocabulary",
        description="출제 의도 — V2 는 항상 'vocabulary'.",
    )

    @model_validator(mode="after")
    def _validate_options_length(self) -> V2InlineChoiceOutput:
        if len(self.options) != 2:
            raise ValueError(
                f"options 는 정확히 2개여야 한다 (correct + distractor). "
...
class V2Output(BaseModel):
    """V2 LLM structured output schema.

    docs/prompts/variant-vocabulary-inline-v0.md 출력 schema 와 1:1 대응.
    ...
    inline_choices: list[V2InlineChoiceOutput] = Field(
        ...,
    ...
    explanation: str = Field(
        ...,
        description="정답 사유 (한국어, 2~3문장).",
    )

    @model_validator(mode="after")
    def _validate_inline_choices_count(self) -> V2Output:
    ...
    @model_validator(mode="after")
    def _validate_choices_length(self) -> V2Output:
...
async def generate_v2_variant(
    *,
    passage_text: str,
    original_question: Question,
    llm_client: StructuredLLMClient,
    prompt_template_id: str = V2_PROMPT_ID,
    box_count: int = _DEFAULT_BOX_COUNT,
) -> Question:
    """V2 vocabulary_inline 변형 생성.

    원본 Passage.body_text 를 기반으로 2~3개 어휘 인라인 박스를 삽입하고,
    ...
    Args:
        ...
        prompt_template_id: 프롬프트 템플릿 ID. 기본 ``variant-vocabulary-inline-v0``.
        box_count: 인라인 박스 개수 (2~3). 기본 2.

    Returns:
        sentinel UUID 가 채워진 신규 Question (variant_kind=VOCABULARY_INLINE).
        라우트가 tenant_id / workspace_id / passage_id / derived_from_question_id 를
        model_copy 로 주입한다.

    Raises:
        ValueError: original_question.type 이 V2 비적용 type (V2_APPLICABLE_TYPES 외).
    ...
    if original_question.type not in V2_APPLICABLE_TYPES:
        raise ValueError(
            f"V2 변형은 vocabulary_30 / blank_phrase_31 type 에만 적용 가능합니다. "
    ...
    result = await llm_client.extract_structured(
        prompt=prompt,
        response_model=V2Output,
        purpose="variant_v2_vocabulary_inline",
    ...
    # V2InlineChoiceOutput → shared.schemas.question.InlineChoice 변환
    ...
    return Question(
        ...
        type=QuestionType.VOCABULARY_30,  # V2 는 vocabulary_30 으로 통일 (카탈로그 v0.4)
        variant_kind=VariantKind.VOCABULARY_INLINE,
        question_text=(
            original_question.question_text or "밑줄 친 (A), (B)에 들어갈 말로 가장 적절한 것은?"
        ),
