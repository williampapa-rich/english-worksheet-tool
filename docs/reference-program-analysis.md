...
## 7. 인터랙션 카탈로그 (P1-0a DoD)

> **출처**: §3 (영상 관찰) + §4.4 (단축키 명세) + 본 카탈로그를 위한 보강 추론.
> **소비자**: P1-2 (인터랙션 UI) 의 컨텍스트 메뉴 / 툴바 / 단축키 설계 입력.
> **상태**: 영상에서 직접 관찰 가능한 항목은 [관찰], 추론은 [추론], PM·와이프
> 인터뷰 필요한 항목은 [미정].
...
| 동작 | 결과 | 출처 |
|---|---|---|
| 텍스트 위 좌클릭 + 드래그 | 단어 단위 선택 (token snap) | [관찰] §3.1 단축키 안내 |
| Shift + 좌클릭 + 드래그 | 기존 선택에 추가 (비연속 multi-selection) | [관찰] §3.1 |
| Alt + 좌클릭 + 드래그 | 기존 선택에서 제거 | [관찰] §3.1 |
| 분석표(하단) 칩 좌클릭 | 본문에서 해당 annotation 하이라이트 / 스크롤 | [추론] 표가 본문 마크와 1:1 — 클릭 네비게이션이 자연스러움. 영상 미관찰. |
| 본문 마크 우클릭 | 컨텍스트 메뉴 (편집 / 삭제 / 카테고리 변경) | [추론] 영상에서 컨텍스트 메뉴 호출 장면 미캡처. P1-2 에서 채택 여부 PM 결정. |
| 본문 마크 더블클릭 | 라벨 텍스트 인라인 수정 모달 | [추론] [미정] |
| 분석표 칩 우클릭 | 삭제 / 카테고리 변경 | [추론] [미정] |

### 7.2 키보드 단축키

| 키 | 동작 | 출처 |
|---|---|---|
| Esc | 편집 취소 (입력 중인 라벨 등) | [관찰] §3.1 |
| Esc Esc (2회) | 모달 닫기 | [관찰] §3.1 |
| Del / Backspace | 선택된 칩 / annotation 삭제 | [관찰] §3.1 |
| Cmd/Ctrl + Z | undo | [추론] [미정] — 영상 미확인. ProseMirror 디폴트 활용 가능. |
| Cmd/Ctrl + Shift + Z | redo | [추론] [미정] |
| Cmd/Ctrl + S | 저장 | [추론] [미정] — Phase 1 P1-6 통합 시점에 결정 |
| 숫자 1~9 | color_index 빠른 선택 (1~9, 10~12 는 Shift + 1~3 등) | [추론] [미정] — 12색 팔레트 빠른 접근 패턴 |

### 7.3 컨텍스트 메뉴 (우클릭)

[추론 + 미정]. 본 카탈로그가 제안하는 1차 항목 — P1-2 에서 PM 검토 후 확정:

| 항목 | 활성 조건 | 동작 |
|---|---|---|
| 라벨 추가 (top) | 텍스트 선택 중 | top_label 입력 모달 |
| 라벨 추가 (bottom) | 텍스트 선택 중 | bottom_label 입력 모달 |
| 하이라이트 적용 | 텍스트 선택 중 | 12색 picker → highlight |
| 밑줄 적용 | 텍스트 선택 중 | underline 토글 |
| 괄호 추가 | 텍스트 선택 중 | bracket style 선택 |
| 화살표 시작점 | 텍스트 선택 중 | arrow start_span 등록 |
| 화살표 끝점 | start_span 이 등록되어 있고 텍스트 선택 중 | arrow end_span 등록 → 화살표 완성 |
| inline_note 추가 | 텍스트 선택 중 | note 텍스트 입력 모달 |
| ─────── | | |
| 카테고리 변경 | 마크 위 우클릭 | 5종 (note/sentence_role/phrase/clause/other) |
| 색 변경 | 마크 위 우클릭 | 12색 picker |
| 라벨 텍스트 수정 | top_label / bottom_label / inline_note 위 우클릭 | 인라인 수정 |
| 삭제 | 마크 위 우클릭 | 해당 annotation 제거 |
...
[관찰 + 추론]. 본문에 마크를 추가하면 하단 분석표의 해당 카테고리 행에 칩이 자동 추가 (영상 §3.3 scene_007 → scene_006 변화).

| 트리거 | 결과 |
|---|---|
| 본문에 annotation 적용 | 분석표의 `category` 행에 칩 추가 (`<라벨 텍스트> <span 텍스트 일부>` 형식) |
| 본문에서 annotation 삭제 | 분석표 칩 제거 |
| annotation `category` 변경 | 분석표에서 행 이동 |
| 분석표 칩 클릭 | 본문 해당 위치로 스크롤 + 강조 [추론] |

[미정 — 와이프 인터뷰 필요]:
- 칩 텍스트 표기 규칙 (라벨 + span 텍스트 줄임 길이)
- 같은 category 내 정렬 순서 (등장 순 / 라벨 알파벳 / 등록 시각)
- 분석표 행 접기/펼치기 가능 여부

### 7.6 P1-2 착수 시 입력으로 확정된 것 / 미정 잔존

**확정 (P1-2 에서 그대로 구현)**:
- 단축키: Esc / Esc Esc / Del
- 12색 picker / 5종 category select 존재
- 마크 적용 후 직렬화는 `SyntaxAnnotation[]` (P1-1 산출물)

**P1-2 드래프트 (PR #16) 에서 합리적 디폴트로 진행한 항목**:
- 단어 단위 선택 → 글자 단위 (디폴트)
- 컨텍스트 메뉴 → 미구현 (툴바만)
- color_index 의미 → 임의 팔레트
- 라벨 입력 → `prompt()` (정식 모달은 후속)
